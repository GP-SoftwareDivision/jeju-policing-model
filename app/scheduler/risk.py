...
def get_traffic_data(conn, link_ids):
    ...
    query = text(f"""
        SELECT 
            ...
            ,extract(month from stat_dt) as month
            ,extract(dow from stat_dt) as day
            ,extract(hour from stat_dt) as hour
            ,cast(tfvl as integer) as tfvl
            ,cast(sped_avg as integer) as sped_avg
        FROM public.info_traffic_hourly_stat A
        JOIN public.tbm_info_link B ON A.link_id = B.link_id
        WHERE B.road_name = '평화로'
        and A.link_id IN ({link_ids})
        and stat_dt = (select max(stat_dt) from public.info_traffic_hourly_stat)
    """)
...
def get_risk():
    ...
    with engine.begin() as conn:       
        ...
        for link_cd in tqdm(link_data['link_code'].unique()):
            ...
            # inference data
            X_test = processed_data.drop(['link_code', 'date'], axis=1)

            # 모델 예측
            pred_proba = model.predict_proba(X_test)  # 클래스별 확률 예측
            pred_labels = model.predict(X_test)  # 최종 예측 클래스

            # 예측 결과를 데이터프레임으로 변환
            pred_df = pd.DataFrame(pred_proba, columns=['probability_0', 'probability_1', 'probability_2'])
            ...
            # 최종 데이터 컬럼명 변경 (한글)
            final_output_df = result_df[['date', 'hour', 'probability_0', 'probability_1', 'probability_2', 'pred', 'tpi', 'tfvl', 'sped_avg', 'link_code']]\
                .rename(columns={'tpi': 'tp'})\
                .assign(hour=lambda df: df['hour'].astype(int)
                    ,tp = lambda df:df['tp'].astype(float)
                    ,tfvl = lambda df:df['tfvl'].astype(str)).reset_index(drop=True)
                
        insert_risk_data(conn, final_output_df, 'info_risk_pred')
